...
//		this.nav.navigationContainer._setSizes();
				...
				if(name == "a" || name == "area"){
					...
					if ( i >=0 ) {
						...
						if ( e != null ) {
							...
							// In Firefox2, scrollIntoView(true) will cause AccordionPane layout mess, disable it in FF2
							if ((dojo.render.html.mozilla)&&(dojo.render.html.UA.indexOf("Firefox/2")>=0)) {
	...
	onContentLoaded:  function(url) {
		if ( this.contentContainer != null ) {
			...
			// Create Manual page view tag on every content change of iframe.
			this.createPageViewTag(url);
		}
	},
	...
	*/
	...
	handlerClick: function(evt){
		...
		if ( url == "#" ) {
			...
			if (node.getAttribute("alphabet") == "true")
			{
	...
	*/
	...
	createPageViewTag : function(url) {
		// Page ID is the part of base URL without "http://"
		var pageURL = this.browser.urlInfo.baseUrl;
		var idx1 = pageURL.indexOf(":") + 3;
		pageURL = pageURL.substring(idx1, pageURL.length - 1);
		
		// Passing the rest of the URL in the search term for now as we do not have any other
		// attribute to store this value in Coremetrics.
		// TODO: Correct this part once we have an attribute set to pass this value to coremetrics
		var paramIdx = url.indexOf("?");
		if(paramIdx > 0) {
			url = url.substring(0, paramIdx);
		}
		if(window.digitalData != null && window.digitalData.page != null && window.digitalData.page.pageInfo != null) {
			window.digitalData.page.pageInfo.pageID = pageURL; // this is mandatory for all pages
			// Set dummy values in Search artifacts - these are required fields.
			window.digitalData.page.pageInfo.onsiteSearchTerm = url;// the actual search keyword should be placed here
			window.digitalData.page.pageInfo.onsiteSearchResult = "1"; // the actual search result should be placed here
