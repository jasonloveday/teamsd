...
dojo.widget.defineWidget(
	...
	dojo.widget.ContentPane,
	{
		...
		_populateTree: function(type, data, evt) {
			// Here we get the complete tree structure
			// which gets displayed in the left navigation
			// with panes as the top level elements.
			// To get only the welcome page
			// we can modify the data here
			for(var i=0; i < data.length; i++){
				var tempdata = data[i];
				var welcomePage = false;
				for(var j=0;j<tempdata.children.length;j++) {
					var child = tempdata.children[j];
					if(child.title == "Welcome") {
